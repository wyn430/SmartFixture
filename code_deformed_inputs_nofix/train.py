...
#def train():
    ...
    max_training_timesteps = int(48001)   # break training loop if timeteps > max_training_timesteps
    ...
    max_num = 20
    ip = ip_addr  ##please change it to the ip of the Ansys server
    port = 8800
    ...
    ppo_agent = PPO(state_dim, action2_dim, shared_channel_list, 
                    actor_arm_dim_list, critic_arm_dim_list, emb_dims, 
                    feature_dims, k, lr_actor, lr_critic, gamma, K_epochs, eps_clip, 
    ...
    log_f.write('PPO K epochs : ' + str(K_epochs) + '\n')
    ...
    while time_step <= max_training_timesteps:
        ...
        original_input_filename = train_file_list[idx]
        ...
        #print('the max deformation is ')
        #print(initial_deform.max())
        
        x, state, mask = env.reset(initial_fixture_locations, original_input_filename, input_filename, initial_deform)
        #print('the max deformation is ')
        #print(state.max())
        #print(state.shape)
        current_ep_reward = 0
        ...
        for t in tqdm(range(1, max_ep_len+1)):
            ...
            x, state, reward, done, mask = env.step(action2, original_input_filename, input_filename, quant_lim, initial_deform)
            #print('the current fixture is')
            #print(action2)
